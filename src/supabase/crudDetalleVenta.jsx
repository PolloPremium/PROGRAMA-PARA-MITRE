...
export async function EditarDescuentoDetalleVenta(p) {
  // Nota: Esta función requiere que la columna 'descuento' exista en la tabla detalle_venta
  // Si no existe, crear una migración en Supabase para agregar la columna
  const { error } = await supabase.from(tabla).update({ descuento: p._descuento }).eq("id", p._id);
  if (error) {
    console.warn("EditarDescuentoDetalleVenta - Posible columna no existe. Error:", error.message);
    // Por ahora, no lanzamos el error para evitar que se rompa la app
    return;
  }
}
