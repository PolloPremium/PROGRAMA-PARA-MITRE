...
export function AreaDetalleventaPos() {
  ...
  const [editDescuentoIndex, setEditDescuentoIndex] = useState(null);
  const [newDescuento, setNewDescuento] = useState(0);
  const { mostrardetalleventa, editarCantidadDetalleVenta,eliminardetalleventa, editarDescuentoDetalleVenta } =
    useDetalleVentasStore();
  ...
  const EditarDescuentoDV = async (data) => {
    const p = {
      _id: data.id,
      _descuento: data.descuento,
    };
    await editarDescuentoDetalleVenta(p);
  };
  ...
  const {mutate:mutateEditarDescuentoDV} = useMutation({
    mutationKey: ["editar descuento detalle venta"],
    mutationFn: EditarDescuentoDV,
    onError:(error)=>{
      toast.error(`Error: ${error.message}`)
    },
    onSuccess:()=>{
          ...
          <Itemventa key={index}>
            ...
            <article className="contentTotaldetalleventa">
              <span className="cantidad">
                <strong>
                  {FormatearNumeroDinero(
                    item.total - (item.descuento || 0),
                    dataempresa?.currency,
                    dataempresa?.iso
                  )}
                </strong>
              </span>
              {editDescuentoIndex === index ? (
                <InputText2>
                  <input
                    type="number"
                    value={newDescuento}
                    onChange={(e) => setNewDescuento(Math.max(0, parseFloat(e.target.value) || 0))}
                    onBlur={() => {
                      mutateEditarDescuentoDV({id: item.id, descuento: newDescuento});
                      setEditDescuentoIndex(null);
                    }}
                    onKeyDown={(e) => {
                      if (e.key === "Enter") {
                        mutateEditarDescuentoDV({id: item.id, descuento: newDescuento});
                ...
                <span 
                  className="descuento"
                  onClick={() => {
                    setEditDescuentoIndex(index);
                    setNewDescuento(item.descuento || 0);
                  }}
                  style={{ cursor: "pointer", fontSize: "12px", color: item.descuento > 0 ? "#ff6b6b" : "#666" }}
                >
                  {item.descuento ? `Desc: ${FormatearNumeroDinero(item.descuento, dataempresa?.currency, dataempresa?.iso)}` : "Desc"}
                </span>
              )}
